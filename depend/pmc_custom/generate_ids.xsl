...
<xsl:stylesheet xmlns:db="http://docbook.org/ns/docbook"
    ...
    <xsl:copy>
      ...
      <xsl:if test="self::db:sect1">       
          <xsl:if test="@xml:id">
           ...
           <xsl:attribute name="xml:id">section_<xsl:value-of select="$title"/><xsl:value-of select="generate-id()"/></xsl:attribute>  
      </xsl:if>
      <xsl:if test="self::db:table">
        <xsl:if test="@xml:id">
          <xsl:attribute name="id"><xsl:value-of select="@xml:id"/></xsl:attribute>
            ...
            <xsl:attribute name="xml:id">table_<xsl:value-of select="$title"/><xsl:value-of select="generate-id()"/></xsl:attribute>
      </xsl:if>
      <xsl:if test="self::db:figure">
        <xsl:if test="@xml:id">
          <xsl:attribute name="id"><xsl:value-of select="@xml:id"/></xsl:attribute>
            ...
            <xsl:attribute name="xml:id">figure_<xsl:value-of select="$title"/><xsl:value-of select="generate-id()"/></xsl:attribute>          
      </xsl:if>
      <xsl:if test="self::db:procedure">
        <xsl:if test="@xml:id">
          <xsl:attribute name="id"><xsl:value-of select="@xml:id"/></xsl:attribute>
            ...
            <xsl:attribute name="xml:id">procedure_<xsl:value-of select="$title"/><xsl:value-of select="generate-id()"/></xsl:attribute>          
      ...
      <xsl:if test="self::db:info">
        <xsl:if test="@xml:id">
          <xsl:attribute name="id"><xsl:value-of select="@xml:id"/></xsl:attribute>
            ...
            <xsl:attribute name="xml:id">info_<xsl:value-of select="$title"/><xsl:value-of select="generate-id()"/></xsl:attribute>
      ...
      <xsl:if test="self::db:bridgehead">
        ...
        <xsl:attribute name="xml:id">bridgehead_<xsl:value-of select="$title"/><xsl:value-of select="generate-id()"/></xsl:attribute>        
      </xsl:if>
  ...
  <xsl:template match="db:section">
        <xsl:copy>
          ...
          <xsl:attribute name="xml:id">section_<xsl:value-of select="normalize-space($title)"/><xsl:value-of select="generate-id()"/></xsl:attribute>  
...
<xsl:template name="check-for-leading-number">  
  ...
  <xsl:template name="generate-IDs">
    ...
    <xsl:variable name="id-being-generated.chars">,/&amp;;:0123456789~`@#$%^*-()+={}[]&gt;&lt;!™®°µ "'.</xsl:variable>
