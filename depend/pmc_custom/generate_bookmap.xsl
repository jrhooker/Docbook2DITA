<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:include href="parameter_structures_ditamap.xsl"/>
        ...
        <xsl:for-each select="db:info">
            <xsl:element name="bookmeta">
                <xsl:element name="pmc_iso">
                    ...
                    <xsl:element name="pmc_productnumber">[PMxxxx]</xsl:element>
                    <xsl:element name="pmc_document_id">[ESC-xxxxxxx]</xsl:element>
                    <xsl:element name="pmc_issue_date">Issue Date</xsl:element>
                    <xsl:element name="pmc_dev_status"><xsl:attribute name="dev_status">Preliminary</xsl:attribute></xsl:element>
                    <xsl:element name="pmc_doc_status">Doc Type</xsl:element>
                    <xsl:element name="pmc_doc_type">Doc Type</xsl:element>
                    <xsl:element name="pmc_issuenum">Issuenum</xsl:element>
                </xsl:element>
            </xsl:element>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template match="db:pmc_productname" mode="info"> </xsl:template>
    ...
    <xsl:template match="db:pmc_dcp_phase" mode="info"> </xsl:template>
    ...
    <xsl:template match="db:pmc_footertext" mode="info"> </xsl:template>
            ...
            <xsl:element name="indexterm">
                ...
                <xsl:element name="indexterm">
                    ...
                    <xsl:element name="indexterm">
                        <xsl:call-template name="attribute_manager"/>
                        <xsl:choose>
                            <xsl:when test="//db:procedure[@xml:id = $temp_id]">Procedure
                                    <xsl:number select="//db:procedure[@xml:id = $temp_id]"
                                    format="001" level="any" count="db:procedure"/>:
                                    <xsl:call-template name="get_index_topic_title"/></xsl:when>
                            ...
                            <xsl:when test="//db:figure[@xml:id = $temp_id]">Figure <xsl:number
                                    select="//db:figure[@xml:id = $temp_id]" format="001"
                                    level="any" count="db:figure"/>: <xsl:call-template
                                    name="get_index_topic_title"/></xsl:when>
                            <xsl:when test="//db:example[@xml:id = $temp_id]">Example <xsl:number
                                    select="//db:example[@xml:id = $temp_id]" format="001"
                                    level="any" count="db:example"/>: <xsl:call-template
                                    name="get_index_topic_title"/></xsl:when>
                        </xsl:choose>
                    </xsl:element>
                </xsl:element>
