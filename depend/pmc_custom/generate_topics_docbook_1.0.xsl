<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:include href="parameter_structures.xsl"/>
            ...
            <xsl:when test="@xml:id">
                ...
                <xsl:result-document href="{$sectionNumber}.xml">
                    <topic id="{$sectionNumber}">
                        ...
                        <xsl:call-template name="topic_attribute_manager"/>
                ...
                <xsl:message>Document created: <xsl:value-of select="$sectionNumber"/></xsl:message>
            </xsl:when>
            <xsl:otherwise>
                <xsl:variable name="sectionNumber">
    ...
    <xsl:template name="topic_title">
        <xsl:for-each select="db:title | db:caption">
            <xsl:choose>
    ...
    <xsl:template match="db:para | db:caption">
        <xsl:choose>
            <xsl:when test="parent::db:table"></xsl:when>
            <xsl:otherwise>
                <xsl:element name="p">
                <xsl:call-template name="attribute_manager"/>
                <xsl:apply-templates/>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:element name="table">
                    ...
                    <xsl:call-template name="topic_title"/>
                    <xsl:choose>
                        <xsl:when test="child::db:tgroup"><xsl:apply-templates/></xsl:when>
                        <xsl:otherwise><xsl:element name="tgroup">
                            <xsl:attribute name="cols"><xsl:value-of select="count(db:colgroup/db:col)"/></xsl:attribute><xsl:apply-templates/></xsl:element></xsl:otherwise>
                    </xsl:choose>                   
                </xsl:element>
                <xsl:call-template name="externalize-tfooters"/>
            </xsl:otherwise>
    ...
    <xsl:template match="db:row | db:tr">        
        <xsl:choose>
    ...
    <xsl:template match="db:entry | db:td | db:th">
        <xsl:choose>
    ...
    <xsl:template match="db:programlisting | db:screen">
        <xsl:variable name="content" select="translate(., '&lt;?linebreak?&gt;', '')"/>       
        <xsl:element name="codeblock">
            <xsl:call-template name="attribute_manager"/>
            <xsl:value-of select="translate(., '&lt;?linebreak?&gt;', '')"/>
        </xsl:element>
    </xsl:template>
...
-->
    ...
    <xsl:template name="topic_attribute_manager">
        <xsl:for-each select="@*">
            <xsl:choose>
                <xsl:when test="name(.) = 'audience'">
